...
use App\Filament\Tsn\Resources\PenerimaanSantriResource\Pages;
use App\Filament\Tsn\Resources\PenerimaanSantriResource\RelationManagers;
use App\Filament\Tsn\Resources\PenerimaanSantriResource\Widgets\JumlahPendaftar;
...
class PenerimaanSantriResource extends Resource
{
    protected static ?string $modelLabel = 'Penerimaan Santri';

    protected static ?string $navigationLabel = 'Penerimaan Santri';

    protected static ?string $pluralModelLabel = 'Penerimaan Santri';
    ...
    {
        return $form
            ->schema(function (Santri $record) {
                // dd($record->jenispendaftar);

                if ($record->jenispendaftar === 'Baru') {
                    return ([
        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('kelas_tujuan')
                    ->label('Kelas Tujuan')
                    ->searchable()
                    ->default(function (Santri $record) {

                        $getkelassantri = KelasSantri::where('santri_id', $record->id)->first();
                        $kelasid = $getkelassantri->kelas_id;
                        $getkelas = Kelas::where('id', $kelasid)->first();
                        $kelas = $getkelas->kelas;


                        if ($record->naikqism === 'naik') {
                            switch (true) {
                                case ($record->qism_detail_id === 3 ||
                                    $record->qism_detail_id === 4 ||
                                    $record->qism_detail_id === 5 ||
                                    $record->qism_detail_id === 6 ||
                                    $record->qism_detail_id === 7):

                                    return ('Kelas 1');

                                    break;

                                case ($record->qism_detail_id === 8):

                                    return ('Mutawasithoh');

                                    break;

                                case ($record->qism_detail_id === 9):

                                    return ('Kelas A');

                                    break;
                            }
                        } elseif ($record->naikqism !== 'naik') {
                            return ($kelas);
                        }
                    })
                    ->sortable(),
            ...
            ->filters([])
            ->actions([
                ViewAction::make('Lihat Data Santri')
    ...
    {
        return [
            'index' => Pages\ListPenerimaanSantris::route('/'),
            // 'create' => Pages\CreatePenerimaanSantri::route('/create'),
            'view' => Pages\ViewPenerimaanSantri::route('/{record}'),
            // 'edit' => Pages\EditPenerimaanSantri::route('/{record}/edit'),
        ];
    }
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        return parent::getEloquentQuery()->where('qism_id', Auth::user()->mudirqism)->where('daftarnaikqism', 'Mendaftar')->orWhereHas('statussantri', function ($s) {
            $s->where('status', 'calon');
